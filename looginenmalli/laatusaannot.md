...
Sanallisilla kaavamääräyksillä tarkoitetaan kaavatietomallissa kaavamääräystä, jolle on annettu Kaavamääräyksen ```lajiksi``` [Sanallinen määräys](http://uri.suomi.fi/codelist/rytj/RY_Kaavamaarays/code/sanallinenMaarays) ja ```arvo```-attribuutille {% include common/moduleLink.html moduleId="kaavatiedot" path="looginenmalli/dokumentaatio/#tekstiarvo" title="TekstiArvo" %} Sanalliset kaavamääräykset on annotoitava [Sanallisen määräyksen laji](<http://uri.suomi.fi/codelist/rytj/RY_Sanallisen_Kaavamaarayksen_Laji/>) -koodein.
{% include common/clause_end.html %}
